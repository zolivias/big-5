  ...
  return (
    <PageShell>
      ...
      <section className="recommendations"><div className="recommendation-heading"><div><p className="section-kicker">Based on your results</p><h2>A few ideas to try.</h2></div><p>Try any that seem useful. You can save them or remove the ones that do not fit.</p></div>
        <div className="recommendation-grid">{items.map((item, index) => <article className="resource-card" key={item.id}><div className="resource-top"><span className="resource-number">0{index + 1}</span><span>{item.duration}</span></div><h3>{item.title}</h3><p>{item.description}</p><div className="why-box"><strong>Why we suggested it</strong><p>{item.why}</p></div><div className="action-line"><span aria-hidden="true">↗</span><p><strong>Try this:</strong> {item.action}</p></div><div className="card-actions"><button onClick={() => toggleBookmark(item.id)} aria-pressed={currentProfile.bookmarks.includes(item.id)}>{currentProfile.bookmarks.includes(item.id) ? "★ Saved" : "☆ Save"}</button><button onClick={() => dismiss(item.id)}>Not for me</button></div></article>)}</div>
