...
import type { ActivityCompletion, EvidenceSourceId, GoalKey, LocalProfile } from "../../lib/types";

const emptyFeedback: Omit<ActivityCompletion, "activityId" | "completedAt"> = { helpful: "somewhat", similar: "maybe", nextStep: "same" };

function ResearchSnapshot({ sourceIds }: { sourceIds: EvidenceSourceId[] }) {
  const primarySource = researchSources[sourceIds[0]];
  const keywords = Array.from(new Set(sourceIds.flatMap((sourceId) => researchSources[sourceId].keywords))).slice(0, 4);
  return <div className="research-snapshot"><div><strong>What the Research Shows</strong><span>{primarySource.citation}</span></div><p>{primarySource.brief}</p><ul aria-label="Research keywords">{keywords.map((keyword) => <li key={keyword}>{keyword}</li>)}</ul></div>;
}
